...
if (!defined('__CLASS_PARSINGHTML__')) {
    ...
    class parsingHTML {
        ...
        function parse() {
                ...
                // sinon, analyser le code
                    ...
                    if ($res) {
                        if (!in_array($res['name'], array('br', 'hr', 'img', 'input', 'link', 'option'))) {
                            if ($res['close']) {
                                if (count($parents) < 1)
                                    CI_HTML2PDF::makeError(3, __FILE__, __LINE__, $res['name']);
                                else if ($parents[count($parents) - 1] != $res['name'])
                                    CI_HTML2PDF::makeError(4, __FILE__, __LINE__, $parents);
                                else
            ...
            if (count($parents))
                CI_HTML2PDF::makeError(5, __FILE__, __LINE__, $parents);
        ...
        function analiseCode($code) {
            ...
            if ($border !== null) {
                if ($border)
                    $param['style'] = 'border: solid ' . $border . ' ' . $color . '; ' . $param['style'];
                else
            ...
            if (!isset($param['num']))
                $param['num'] = $this->level[count($this->level) - 1];
                ...
                // sinon, c'est une balise html
                    ...
                    if ($row['name'] == $detect) {
                        ...
                        if ($level == 0) {
                            $not = true;
                            $end = true;
                        } // si on est au niveau 0 : on a fini
                    }
                    ...
                    if (!$not) {
                        ...
                        foreach ($row['param'] as $key => $val) {
                            if ($key == 'style') {
                                $tmp = '';
                                if (isset($val['text-align']))
                                    unset($val['text-align']);
