...
if (!defined('__CLASS_STYLEHTML__')) {
    ...
    class styleHTML {
        ...
        function getLastAbsoluteX() {
            for ($k = count($this->table); $k > 0; $k--) {
                if ($this->table[$k - 1]['x'] && $this->table[$k - 1]['position'])
                    return $this->table[$k - 1]['x'];
            }
        ...
        function getLastAbsoluteY() {
            for ($k = count($this->table); $k > 0; $k--) {
                if ($this->table[$k - 1]['y'] && $this->table[$k - 1]['position'])
                    return $this->table[$k - 1]['y'];
            }
        ...
        function getReccursiveStyle($key, $lst, $next = null) {
            ...
            if ($next !== null) {
                if ($next)
                    $key = trim(substr($key, 0, -strlen($next))); // on el�ve cette etape
                unset($lst[0]);
                if (!count($lst))
                    return false; // pas d'etape possible
            ...
            foreach ($lst[0] as $nom) {
                if ($key == $nom)
                    return true; // si la clef conrrespond => ok
                if (substr($key, -strlen(' ' . $nom)) == ' ' . $nom && $this->getReccursiveStyle($key, $lst, $nom))
                    return true; // si la clef est la fin, on analyse ce qui pr�c�de
            }
        ...
        function readBorder($val) {
            ...
            $res = null;
            foreach ($val as $key) {
                if ($key == 'none' || $key == 'hidden')
                    return $none;

                if ($this->ConvertToMM($key) !== null)
                    $width = $this->ConvertToMM($key);
                else if (in_array($key, array('solid', 'dotted', 'dashed')))
                    $type = $key;
                else {
        ...
        function ConvertBackgroundColor($val) {
            ...
            if ($val == 'transparent')
                ...
                return $this->ConvertToRVB($val, $res);
        }
        ...
        function ConvertBackgroundPosition($val, &$res) {
            ...
            if ($val[0] == 'left')
                ...
                $y = '100%';
            else if (preg_match('/^[-]?[0-9\.]+%$/isU', $val[0]))
                $x = $val[0];
            else if ($this->ConvertToMM($val[0]))
                $x = $this->ConvertToMM($val[0]);
            else
                $res = false;
            ...
            if ($val[1] == 'left')
                ...
                $y = '100%';
            else if (preg_match('/^[-]?[0-9\.]+%$/isU', $val[1]))
                $y = $val[1];
            else if ($this->ConvertToMM($val[1]))
                $y = $this->ConvertToMM($val[1]);
            else
                $res = false;
        ...
        function ConvertToRVB($val, &$res) {
            ...
            if (preg_match('/rgb\([\s]*([0-9%]+)[\s]*,[\s]*([0-9%]+)[\s]*,[\s]*([0-9%]+)[\s]*\)/isU', $val, $match)) {
                $r = $match[1];
                if (substr($r, -1) == '%')
                    $r = floor(255 * substr($r, 0, -1) / 100);
                $v = $match[2];
                if (substr($v, -1) == '%')
                    $v = floor(255 * substr($v, 0, -1) / 100);
                $b = $match[3];
                if (substr($b, -1) == '%')
                    $b = floor(255 * substr($b, 0, -1) / 100);
            }
            ...
            foreach ($match[1] as $code) {
                ...
                if (isset($tmp['type']) && strtolower($tmp['type']) == 'text/css' && isset($tmp['href'])) {
                    ...
                    if (strpos($url, 'http://') !== false) {
                        ...
                        $url_main = 'http://' . $url[0] . '/';
                        $url_self = $url;
                        unset($url_self[count($url_self) - 1]);
                        $url_self = 'http://' . implode('/', $url_self) . '/';
