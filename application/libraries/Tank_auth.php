...
class Tank_auth {
    ...
    function __construct() {
        ...
        $this->ci->load->model('tank_auth/Login_attempts','LA');//added for loging IP
        // Try to autologin
        $this->autologin();
    }
                ...
                if ($hasher->CheckPassword($password, $user->password)) {  // password ok
                    if ($user->banned == 1) {         // fail - banned
                        ...
                        $this->ci->session->set_userdata(array(
                            ...
                            'office_block_id' => $user->office_block_id,
                            'office_block_name' => $user->block_name,
                            ...
                            'edudistrict_id' => $user->edudistrict_id,
                            'school_code' => $user->school_code,
                            'school_management_id' => $user->school_management_id,
                            'designation_id' => $user->designation_id,
                            'photo_url' => $user->photo_url,
                            'name' => $user->name,
                            'status' => ($user->activated == 1) ? STATUS_ACTIVATED : STATUS_NOT_ACTIVATED,
                        ));     
                        //Following function is used for checking whether the user changed the default password
                         $isFirstLogin=$this->ci->LA->getInitLoginStatus($user->username);
...
//                            "school_management_id" => $user->school_management_id);
//
//                        $token = JWT::encode($payload, "VmYq3t6w9zuC&F)J");
                        //echo $token;exit;

                        //$this->ci->session->set_userdata(array('token' => $token));
                        //echo $this->ci->session->userdata('token');exit;

                        if ($user->activated == 0) {       // fail - not activated
                            $this->error = array('not_activated' => 'The user is banned or not yet activated');
                        } else {            // success
                            if ($remember) {
    ...
    function logout() {
        ...
        $this->ci->session->set_userdata(array('user_id' => '', 'username' => '', 'status' => '', 'permissions' => '', 'userscope' => '', 'managing' => '', 'usergroup_id' => '', 'SCHOOL_CODE' => '', 'SCHOOL_ID' => '', 'USER_GROUP' => '', 'SCHOOL_TYPE' => '', 'FINANCE_TYPE' => '', 'USER_NAME' => ''));
    ...
    function is_logged_in($activated = TRUE) {
        ...
        $user       = $this->ci->users->check_activated($username);
        $status     = ($user->activated == 1) ? STATUS_ACTIVATED : STATUS_NOT_ACTIVATED;
        return $status;
        //
     ...
     * user_id, username, password, email, new_email_key (if any).
     *
     * @modified by Mohamed Rashid C <http://twitter.com/rashivkp>
     * @param	string
     * @param	string
     * @param	string
     * @param	bool
     ...
     * @return	array
     */
    function rbac_create_user($username, $email, $password, $email_activation, $usergroup, $managing = 0, $office, $designation, $district, $edudistrict, $subdistrict, $school, $schoolmanagement, $profile_name, $mobile,$pen, $office_block_id,$zone_id) {
//		echo "parent :: ".$username;exit;
        if ((strlen($username) > 0) AND ! $this->ci->users->is_username_available($username)) {
            $this->error = array('username' => 'Duplicate Username');
            return NULL;
        }
        if (!$this->ci->users->is_email_available($email)) {
            $this->error = array('email' => 'Duplicate E-Mail');
            return NULL;
        }
        if (!$this->ci->users->is_pen_available($pen) AND $office != 1012) {
            $this->error = array('pen' => 'Duplicate PEN');
            return NULL;
        }
        // Hash password using phpass
        $hasher = new PasswordHash(
                $this->ci->config->item('phpass_hash_strength', 'tank_auth'), $this->ci->config->item('phpass_hash_portable', 'tank_auth'));
        $hashed_password = $hasher->HashPassword($password);
       if($office_block_id == '' || $office_block_id == NULL){
           $office_block_id = 0;
       }
        $data = array(
            'username' => $username,
            ...
            'office_id' => $office,
            'office_block_id' => $office_block_id,
            'zone_id' => $zone_id,
            'designation_id' => $designation,
            'district_id' => $district,
            'subdistrict_id' => $subdistrict,
            'edudistrict_id' => $edudistrict,
            'school_code' => $school,
            'school_management_id' => $schoolmanagement,
        ...
        $res = $this->ci->users->create_user($data, TRUE,$profile);
        if (is_null($res)) {
            return NULL;
        }
    ...
    function rbac_check_active_users($office, $office_block, $usergroup, $district, $eduDistrict, $subDistrict, $zones=NULL, $exclude_user=NULL){
        $users = $this->ci->users->rbac_check_active_users($office, $office_block, $usergroup, $district, $eduDistrict, $subDistrict, $zones, $exclude_user);
        return $users;
    }
